...
- 상품 설명(description)에 "자유 여행", "자유", "쇼핑", "면세", "야시장", "시장", "아울렛" 등의 소비 관련 키워드가 하나라도 있으면 반드시 get_exchange_rate 툴을 호출하세요. 광고 문구 생성 전에 먼저 호출해야 합니다.
- destination_currency는 카테고리와 상품명·설명을 보고 추론할 것 (예: 일본→JPY, 중국→CNY, 유럽→EUR, 태국→THB, 베트남→VND, 호주→AUD, 미국→USD)
- 툴 반환값의 is_favorable이 true인 경우에만 body에 환율 문구를 자연스럽게 1문장 삽입하세요.
  예시: "지금 엔저라 현지에서 쓸 돈이 더 생겼어요 💴"
  예시: "달러 약세라 쇼핑하기 딱 좋은 타이밍이에요 🛍️"
- is_favorable이 false면 환율은 절대 언급하지 마세요.
